...
const gridImages = Array.from({ length: 8 }, (_, i) =>
  `/images/room_zitronenfalter/leos_bergwelt_ferienwohnung_oberstdorf_tiefenbach_zitronenfalter_${i + 1}.webp`
);

// Weitere Bilder (Bilder 9–12)
const moreImages = Array.from({ length: 4 }, (_, i) =>
  `/images/room_zitronenfalter/leos_bergwelt_ferienwohnung_oberstdorf_tiefenbach_zitronenfalter_${i + 9}.webp`
);
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                {moreImages.map((image, index) => (
                  ...
                  >
                    <Image
                      src={image}
                      alt={`Zimmer Zitronenfalter - weiteres Bild ${index + 1}`}
                      fill
                      className="object-cover transition-transform duration-300 group-hover:scale-105"
